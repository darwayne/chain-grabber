...
func main() {
	...
	_, _ = proxyUser, proxyPass

	if *address == "" {
		panic("address required")
	}

	m := txmonitor.NewZeroMonitor(passwords.TxSubHost)
	...
	var rawCli *noderpc.Client
	if raw, ok := cli.(*noderpc.Client); ok {
		rawCli = raw
	}

	errChan := make(chan error, 1)
	//mempoolspace.NewRest(mempoolspace.WithNetwork(params))
	spender, err := memspender.New(m.Subscribe(), !isMainNet, getPublisher(!isMainNet, l, rawCli), *address, l, cli)
	if err != nil {
		panic(err)
	}
...
func getPublisher(isTestNet bool, logger *zap.Logger, cli *noderpc.Client) *broadcaster.Broker[*string] {
	...
	go func() {
		...
		for {
			select {
			case txPointer := <-channel:
				tx := txhelper.FromString(*txPointer)
				if tx == nil {
					continue
				}
				hash, err := cli.BroadcastTx(ctx, tx)
				if err != nil {
					logger.Info("error broadcasting", zap.String("err", err.Error()))
				} else if hash != nil {
					logger.Info("successfully broadcast transaction", zap.String("broadcast_tx_id", hash.String()))
				}
			}
		}
	}()
